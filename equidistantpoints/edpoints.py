...
class EquidistantPoints(object):
    ...
    def __init__(self, n_points, equatorial_radius=6378137.0, polar_radius=6356752.3):
        ...
        Args:
            ...
            equatorial_radius (float): Earth's radius on the equator in meters (default taken from
                                       WGS-84 system)
    ...
    def __write_to_csv(self, file_path, coord_type, header=None):
        ...
        Args:
            file_path (str): Path to which the CSV should be written
            coord_type (str): The coordinate type to be written ('geodetic' | 'cartesian' | 'ecef')
            header (List[str]): The header row to be written
        """
    ...
    def write_geodetic_to_csv(self, file_path, header=True):
        ...
        Args:
            ...
            header (bool): Indicates if a header row shall be written
        """
        if header:
